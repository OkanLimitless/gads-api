...
const LANGUAGE_CRITERIA_MAP: Record<string, number> = {
  ...
  'cs': 1023, // Czech
  'da': 1009, // Danish
  'nl': 1010, // Dutch - FIXED! Was 1019 (Arabic)
  'en': 1000, // English
  'et': 1043, // Estonian
  'fi': 1015, // Finnish
  'fr': 1002, // French
...
export async function createCampaign(
  ...
  try {
    ...
    // Step 0: Validate account accessibility and status
    console.log(`🔍 Validating account ${customerId} accessibility...`)
    try {
      ...
      return {
        success: false,
        error: `Account validation failed: ${validationError.message || 'Account not accessible'}`
      }
    }
    ...
    try {
      ...
      // ✅ FIXED: Dutch language constant corrected from 1019 (Arabic) to 1010 (Dutch)
      ...
      const languageOperations = [
        {
          ...
          resource: {
            ...
            campaign: campaignResourceName,
            language: {
              language_constant: ResourceNames.languageConstant(languageConstantId.toString()),
            },
          },
        },
      ]
